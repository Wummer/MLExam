...
def jaakkola(X_train,y_train):
	...
	for i,n in enumerate(y_train):
		...
		for j,k in enumerate(y_train):
			if n!=k:
				D = np.sqrt(X_train[i]-X_train[j]**2)
				np.append(temp,D)
		#Ascending order		
		np.sort(temp)
		G.append(temp[0])
...
def SVM_Gridsearch(X_train, y_train, folds):
	...
	tuned_parameters = [{'gamma': [y_jaakkola * np.exp(-3), y_jaakkola*np.exp(-2),y_jaakkola*np.exp(-1),y_jaakkola*np.exp(0),
	 y_jaakkola*np.exp(1), y_jaakkola*np.exp(2),y_jaakkola*np.exp(3)],
